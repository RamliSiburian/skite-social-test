...
export function parseObjectToArrays (dataObj: any, options: { includeProperty?: boolean }) {
  const keyMapper = Object.keys(dataObj)

  return keyMapper.map(a => (
    options?.includeProperty ? 
      `${a}:@:${dataObj[a]}` : dataObj[a]
  ))
}
